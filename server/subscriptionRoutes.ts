...
import { verifyPlayStorePurchase, handlePlayStoreWebhook } from "./services/googlePlayService";
...
export function registerSubscriptionRoutes(app: Express): void {
  ...
  // Google Play Billing: verify a purchase token after user subscribes in the Android app
  app.post("/api/play/verify", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!purchaseToken || !subscriptionId) {
        return res.status(400).json({ error: "purchaseToken and subscriptionId are required" });
      }

      const result = await verifyPlayStorePurchase(purchaseToken, subscriptionId, userId);
      if (result.valid) {
        res.json({ success: true, expiresAt: result.expiresAt });
      } else {
        res.status(400).json({ success: false, error: result.error || "Purchase not valid" });
      }
    } catch (error) {
  ...
  // Google Play Billing: webhook from Google Cloud Pub/Sub for subscription lifecycle events
  app.post("/api/play/webhook", async (req, res) => {
    try {
      const { message } = req.body;
      if (!message?.data) {
        return res.status(400).json({ error: "Invalid Pub/Sub message" });
      }
      await handlePlayStoreWebhook(message.data);
      res.json({ success: true });
    } catch (error) {
