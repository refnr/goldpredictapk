...
// Prediction limits per subscription plan — Pro is the only paid plan, unlimited
...
// Bypass emails: these get free unlimited access without a subscription
const BYPASS_EMAILS: Record<string, string> = {
  "furlan27.mattia@gmail.com": "pro",
};

// Check if user has an active subscription (or is a bypass email)
function userHasAccess(user: any): boolean {
  if (BYPASS_EMAILS[user.email]) return true;
...
export async function registerRoutes(
  ...
  // Live price endpoint — no auth required so the landing page can fetch it if needed
  ...
  app.post(api.prediction.analyze.path, async (req, res) => {
    try {
      ...
      // Subscription check — must have active subscription (unless bypass email)
      if (!userHasAccess(user)) {
        return res.status(403).json({
          message: 'An active subscription is required to make predictions.',
      ...
      if (usedToday >= limit) {
        return res.status(429).json({
          message: `Daily prediction limit reached. Please try again tomorrow.`,
          code: 'LIMIT_EXCEEDED',
          limit,
          used: usedToday,
        });
  ...
  // Signal generation — requires active subscription
  app.post(api.signals.generate.path, isAuthenticated, async (req, res) => {
    try {
      ...
      // Subscription check — Pro plan required for signals
      if (!userHasAccess(user)) {
        return res.status(403).json({
          message: 'An active Pro subscription is required for trading signals.',
          code: 'SUBSCRIPTION_REQUIRED',
        });
      }
